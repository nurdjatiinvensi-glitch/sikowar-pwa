import type { AnnouncementItem } from "./types";

export const announcementData: AnnouncementItem[] = [
  {
    id: 1,
    type: "event",
    eventId: 1,

    title: "Kerja Bakti Minggu",
    date: "Minggu, 8 Juni 2026",
    time: "08:00 - 11:30 WIB",
    location: "Halaman Balai Warga RT 02",

    description:
      "Seluruh warga diharapkan hadir untuk kegiatan kerja bakti membersihkan lingkungan dan saluran air.",
  },

  {
    id: 2,
    type: "event",
    eventId: 2,

    title: "Pertemuan Warga",
    date: "Sabtu, 14 Juni 2026",
    time: "19:30 - 21:00 WIB",
    location: "Sekretariat Warga RT 02",

    description:
      "Pertemuan rutin bulanan untuk membahas kegiatan warga dan agenda lingkungan.",
  },

  {
    id: 3,
    type: "event",
    eventId: 3,

    title: "Imunisasi Balita",
    date: "Minggu, 15 Juni 2026",
    time: "07:30 - 11:00 WIB",
    location: "Posyandu Warga RT 02",

    description:
      "Program imunisasi balita bekerja sama dengan Puskesmas setempat.",
  },

  {
    id: 4,
    type: "announcement",

    title: "Perubahan Jadwal Posyandu",
    date: "Senin, 16 Juni 2026",
    time: "-",
    location: "RT 02",

    description:
      "Pelayanan Posyandu tanggal 16 Juni dipindahkan ke Balai Warga RT 02.",
  },

  {
    id: 5,
    type: "news",

    title: "RT 02 Raih Juara Kampung Bersih",
    date: "Selasa, 17 Juni 2026",
    time: "-",
    location: "Kelurahan Sukamaju",

    description:
      "RT 02 berhasil meraih penghargaan Kampung Bersih tingkat kelurahan berkat partisipasi aktif seluruh warga.",
  },
];